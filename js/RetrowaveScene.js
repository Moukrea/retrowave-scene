...
export var RetrowaveScene = function ()
{
    ...
    this.svgFiles = [
        [ 'js/retrowave_files/sun.svg', 0, 40, -500, 0.11, 'sun' ],
        [ 'js/retrowave_files/city_far.svg', 0, 15, -450, 0.4, 'cityFar' ],
        [ 'js/retrowave_files/city_close.svg', 0, 28, -300, 0.2, 'cityClose' ]
    ];
    // ... and specify your skybox textures here
    this.skybox = [
        `js/retrowave_files/skybox/${ this.textureResolution }/px.png`, // X+
        `js/retrowave_files/skybox/${ this.textureResolution }/nx.png`, // X-
        `js/retrowave_files/skybox/${ this.textureResolution }/py.png`, // Y+
        `js/retrowave_files/skybox/${ this.textureResolution }/invisible.png`, // Y- (out of camera FoV here)
        `js/retrowave_files/skybox/${ this.textureResolution }/invisible.png`, // Z+ (out of camera FoV here)
        `js/retrowave_files/skybox/${ this.textureResolution }/nz.png`, // Z-
    ];
...
RetrowaveScene.prototype.prepareScene = function ( wantProceduralSky = false, wantAnimation = false )
{
    ...
    if ( wantProceduralSky ) { this.addProceduralSky(); }
    else { this.addSkybox(); }
...
{
    let roadLineLeftGeometry = new THREE.PlaneBufferGeometry( .35, 300, 0, 0 );
    roadLineLeftGeometry.translate( -5.2, 110, 0.2 );
    roadLineLeftGeometry.rotateX( -Math.PI * 0.5 );

    let roadLineRightGeometry = new THREE.PlaneBufferGeometry( .35, 300, 0, 0 );
    roadLineRightGeometry.translate( 5.2, 110, 0.2 );
    roadLineRightGeometry.rotateX( -Math.PI * 0.5 );

    let roadLineCenterLeftGeometry = new THREE.PlaneBufferGeometry( .15, 300, 0, 0 );
    roadLineCenterLeftGeometry.translate( -1.8, 110, 0.2 );
    roadLineCenterLeftGeometry.rotateX( -Math.PI * 0.5 );

    let roadLineCenterRightGeometry = new THREE.PlaneBufferGeometry( .15, 300, 0, 0 );
...
RetrowaveScene.prototype.addSidewalk = function ()
{
    ...
    let sidewalkSideLeftGeometry = new THREE.PlaneBufferGeometry( .5, 300, 0, 0 );
    sidewalkSideLeftGeometry.translate( .06, 110, 6 );
    ...
    let sidewalkSideRightGeometry = new THREE.PlaneBufferGeometry( .5, 300, 0, 0 );
    sidewalkSideRightGeometry.translate( .44, 110, -6 );
...
RetrowaveScene.prototype.addGroupedPyramids = function ()
{
    ...
    for ( let i = 0; i < 80; i++ )
    {
        ...
        if ( i % 2 == 0 ) 				// For left side, make translateX negative as 0 is the center
        {
            minTranslateX *= -1;
            maxTranslateX *= -1;
        }
        ...
        function checkPyramidHistory ()
        {
            ...
            if ( checkHistory == true )
            {
                ...
                translateX = this.randomize( minTranslateX, maxTranslateX, 'float' );
                translateZ = this.randomize( minTranslateZ, maxTranslateZ, 'float' );
                if ( this.count < 5000 ) 		// While new values are overlaping with existing pyramids values,
                {
                    let check = checkPyramidHistory.bind( this );
                    check();	// generate new values and check if they overlap.
                }
                else								// If after 10000 checks and random values generated, it still overlap with existing graphics,
                {
...
RetrowaveScene.prototype.checkPositionHistory = function ( sizeOfGeometry, currentPositionX, currentPositionZ )
{
    ...
    {
        if (	// if current position + current size overlaps with previously set positions (-/+) their own size 
 ...
 */
RetrowaveScene.prototype.loadSvgGraphics = async function ( svgUrl, positionX = 0, positionY = 0, positionZ = 0, scale = 1, objectName = 'svg' )
{
    ...
    // Await for load, then...
    await promiseLoader.load( svgUrl )
        .then( ( data ) =>
        {
...
RetrowaveScene.prototype.addSvgGraphics = async function ()
{
    ...
    {
        await this.loadSvgGraphics( this.svgFiles[ i ][ 0 ], this.svgFiles[ i ][ 1 ], this.svgFiles[ i ][ 2 ], this.svgFiles[ i ][ 3 ], this.svgFiles[ i ][ 4 ], this.svgFiles[ i ][ 5 ] );
    }
}
 ...
 */
RetrowaveScene.prototype.prepareShader = function ( shader, value1, value2, transformedY = '1.', scale = '3.', wantFlip = false )
{
    let transformedX = '';
    if ( wantFlip ) { transformedX = 'transformed.x *= sign(instPosition.x);' }
...
RetrowaveScene.prototype.animate = function ()
{
    ...
    if ( this.fpsCounterIsActive ) { this.stats.update(); }
...
{
    if ( this.glitchPass.enabled == false ) // Enables glitch pass every 30 seconds
    {
    ...
    else if ( this.glitchPass.enabled == true ) // Disables glitch pass after 5 seconds
    {
...
RetrowaveScene.prototype.randomize = function ( min, max, setting )
{
    ...
    else if ( setting == 1 || setting == 2 )
    {
        ...
        if ( randomResult == this.previousOddRandomInteger || randomResult == this.previousEvenRandomInteger )
        {
            if ( setting == 1 )
            {
                if ( randomResult < this.previousOddRandomInteger + 1 && randomResult > this.previousOddRandomInteger - 1 )
                {
                    do
                    {
                        randomResult = Math.floor( Math.random() * ( max - min + 1 ) ) + min;
                    }
                    while ( randomResult < this.previousOddRandomInteger + 1 && randomResult > this.previousOddRandomInteger - 1 )
                }
            ...
            {
                if ( randomResult < this.previousEvenRandomInteger + 1 && randomResult > this.previousEvenRandomInteger - 1 )
                {
                    do
                    {
                        randomResult = Math.floor( Math.random() * ( max - min + 1 ) ) + min;
                    }
                    while ( randomResult < this.previousEvenRandomInteger + 1 && randomResult > this.previousEvenRandomInteger - 1 )
                }
